...
function Rooms() {
  return (
    <section id="rooms" style={{ padding: "5rem 1.5rem", background: "#fff" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "1.5rem" }}>
          {rooms.map((room, i) => (
            <div key={i} style={{ background: "#fff", border: "0.5px solid rgba(186,117,23,0.2)", borderRadius: "16px", overflow: "hidden" }}>
              {/* Image placeholder */}
              <div style={{ height: "clamp(160px, 25vw, 200px)", background: room.gradient, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <p style={{ color: "rgba(250,234,218,0.35)", fontSize: "11px", letterSpacing: "3px" }}>{room.name.toUpperCase()}</p>
              </div>
              ...
              <div style={{ padding: "1.25rem 1.5rem" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "1.25rem", paddingTop: "1rem", borderTop: "0.5px solid rgba(186,117,23,0.15)" }}>
                  <div>
                    <span style={{ fontSize: "17px", fontWeight: "500", color: "#BA7517" }}>{room.price}</span>
                    <span style={{ fontSize: "12px", color: "#888780", marginLeft: "4px" }}>/ night</span>
                  </div>
                  <a href="#booking" style={{ background: "#BA7517", color: "#FAEEDA", padding: "8px 18px", borderRadius: "4px", fontSize: "13px", textDecoration: "none" }}>
                    Book
                  </a>
                </div>
              </div>
